...
// Cache interno para getPosts - com chave dinâmica
async function getPostsInternal({ page = 1, limit = 12, status, categoryId } = {}) {
  const db = getDb();
...
export async function getPosts(options = {}) {
  ...
  const cacheKey = `posts-${options.page || 1}-${options.limit || 12}-${options.status || 'all'}-${options.categoryId || 'all'}`;

  const getPostsCached = unstable_cache(
    async () => getPostsInternal(options),
    [cacheKey],
...
export async function getLatestPosts(limit = 5) {
  return getLatestPostsCached(limit);
}
